...
Insertion Delay (ID): Delay from clock root to a register clock input.
ID-M: Insertion delay inside the MGMT core. 
ID-O: Insertion delay, from clock root to MGMT core output
ID-U: Insertion delay inside the USER core (use_project_wrapper). 
...
The MGMT core outputs are valid at the rising edge of clk.
The USER core captures the signals at falling edge.
The User core sets its outputs valid based on the falling edge.
The MGMT core captures the incoming signals at the rising edge.
